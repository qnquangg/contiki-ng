...
def main(log_filepath):
    ...
    sender_pattern = re.compile(
        r"^(?P<time>\S+)\s+ID:22\s+Send to multicast address:\s+\S+,\s+data\s+\[(?P<data>0x[0-9a-fA-F]+)\],\s+packet_number\s+#(?P<packet_number>\d+)"
    )
    ...
    receiver_data = {rid: {'max_total_received': 0,
                           'last_entry': None,
                           'latencies': []} for rid in range(1, 22)}
    ...
    with open(log_filepath, "r") as f:
        for line in f:
            ...
            if receiver_match:
                ...
                if receiver_id < 1 or receiver_id > 21:
                    continue
