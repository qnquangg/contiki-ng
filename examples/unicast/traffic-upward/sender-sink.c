...
/*---------------------------------------------------------------------------*/
PROCESS(root_process, "Sender / RPL Sink");
...
/*---------------------------------------------------------------------------*/
enum send_status_t send_unicast_to_neighbors()
{
  ...
  sprintf(payload, "Send from sink with packet_number %u", packet_number);

  simple_udp_sendto(&udp_conn, payload, strlen(payload), &child_ips[child_idx]);

  LOG_INFO_("Send to node ID:%d, address is ", child_idx+1);
